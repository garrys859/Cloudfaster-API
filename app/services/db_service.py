...
class DatabaseService:
    ...
    def log_docker_service_creation(self, userid, service_name, webtype_id, status="active"):
        query = """
        INSERT INTO docker_services (userid, service_name, webtype_id, status)
        VALUES (%s, %s, %s, %s)
        """
        self.execute_query(query, (userid, service_name, webtype_id, status))
    ...
    def get_docker_service(self, userid: int, service_name: str):
        query = """
        SELECT id, webtype_id, status
        FROM docker_services
        WHERE userid = %s AND service_name = %s
        """
        return self.fetch_one(query, (userid, service_name))
