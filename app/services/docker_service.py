...
class DockerService:
    ...
    def _ensure_path(self, userid, service_name):
        user_info = self.db_service.get_user_by_userid(userid)
        if user_info:
            username = user_info[1]
            target = self.base_path / "users" / str(username) / str(service_name)
        else:
            target = self.base_path / "users" / str(userid) / str(service_name)
    ...
    def create_service(self, userid, webname, service_type, zip_path=None, admin_pass="admin123"):
        target = self._ensure_path(userid, webname)
        if zip_path:
            self._safe_extract(zip_path, target / "data")
            os.remove(zip_path)
        self._init_filebrowser(target, admin_pass)
        template = DOCKER_TEMPLATES.get(service_type)
        if not template:
        ...
        webtype_id = self.db_service.get_webtype_id(service_type)
