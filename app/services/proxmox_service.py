...
import mysql.connector  # For catching IntegrityError
...
class ProxmoxService:
    ...
    def clone_vm_atomic(
        self, userid: int, node: str, template_id: int, vm_name: str, os: str,
        cores=2, memory=2048, ssh_pub_key=None, max_retries=5, template_node="sv1"
    ):
        self._connect()
        ...
        while attempt < max_retries:
            ...
            try:
                ...
                continue
            try:
                self.proxmox.nodes(template_node).qemu(template_id).clone.post(
                    newid=vm_id,
                    target=node,
                    name=safe_vm_name
                )
    ...
    def control_vm(self, vm_id, action, node="sv4"):
        self._connect()
        try:
            if action == "start":
                self.proxmox.nodes(node).qemu(vm_id).status.start.post()
    ...
    def get_free_vmid(self, node="sv4", start=1000, end=9999):
        self._connect()
